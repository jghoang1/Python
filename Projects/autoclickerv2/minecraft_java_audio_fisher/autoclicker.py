...
class AutoClicker():
    ...
    def __init__(self):
        ...
        # Rows 0-1 Title
        ttk.Label(self.frm, text="An Autoclicker by Julius").grid(column=0, row=0, columnspan=COLUMNS)
        ttk.Label(self.frm, text="press END to quit. Press ` to play/pause\n").grid(column=0, row=1, columnspan=COLUMNS)
...
class SequenceRow(AutoclickerRow):
    ...
    def callback(self):
        ...
        key, duration = self.sequence[self.seq_idx]
        prev_key, _ = self.sequence[(self.seq_idx - 1) % len(self.sequence)]
        self.logger.info(f"Sequence callback. KeyUp: {prev_key}")
        self.logger.info(f"Sequence callback. keyDown: {key}")

        self.func_stringvar.set(f"{self.original_label} : {key}")
        pydirectinput.keyUp(prev_key)
        pydirectinput.keyDown(key)
        self.timer.set_duration(duration)

        self.seq_idx = (self.seq_idx + 1) % len(self.sequence)
        self.logger.info(f"Sequence callback. new seq_idc: {self.seq_idx}")
